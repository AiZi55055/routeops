...
import { ref, set } from 'firebase/database';         // ✅ from firebase/database
import { rtdb } from '@/lib/firebase';                // ✅ our RTDB handle
import { useMessengerLocation } from './hooks/useMessengerLocation';

type Props = { messengerId: string };

export default function LiveTracker({ messengerId }: Props) {
  const [on, setOn] = useState(false);
  const { error } = useMessengerLocation({ messengerId, enabled: on });
  ...
  async function testWrite() {
    try {
      ...
      await set(
        ref(rtdb, `locations/${messengerId}`),
        {
          ts: Date.now(),
          lat: 55.755826,
          lng: 37.6173,
  ...
  const toggle = async () => {
    ...
    setOn(next);
    // mark status
    const dref = doc(db, 'messengers', messengerId);
    await updateDoc(dref, { status: next ? 'on' : 'off' });
  };
  ...
  return (
    <div className="p-4 space-y-3">
      ...
      <button
        className={`w-full py-3 rounded ${on ? 'bg-red-600 text-white' : 'bg-green-600 text-white'}`}
        onClick={toggle}
      >
